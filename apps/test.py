...
from apps.train import load_pretrained_model
from assessment.metrics import Metrics
from common.visualizer import Visualizer
from media.codecs import ImageCoder
from common.coding_utils import frame2tensor, tensor2frame
from entropy import KpEntropyCoder
from media.anchors import HEVC, VVC_VTM, VvenC
...
class Inputs:
    ...
    def create_gops(self):
        if self.video is None:
            raise ValueError("Inputs.video must be set before create_gops()")
        video = self.video
        num_gops = max(1, self.num_frames // self.gop_size)
        for idx in range(num_gops):
            self.gops.append(video[idx * self.gop_size: idx * self.gop_size + self.gop_size])
...
class ConventionalCodec:
    ...
    def create_gops(self):
        if self.video is None:
            raise ValueError("ConventionalCodec.video must be set before create_gops()")
        video = self.video
        num_gops = max(1, self.num_frames // self.gop_size)
        for idx in range(num_gops):
            self.gops.append(video[idx * self.gop_size: idx * self.gop_size + self.gop_size])
...
def resize_video_if_needed(video, target_h, target_w):
    ...
    if h != target_h or w != target_w:
        return np.array([np.array(Image.fromarray(video[i]).resize(
            (target_w, target_h), Image.Resampling.LANCZOS)) for i in range(N)])
    return video
...
def predictive_coder(models, input_data, visualizer, out_path='results'):
    ...
    for gop in input_data.gops:
        ...
        with torch.no_grad():
            ...
            prev_latent = None
            prev_res_hat: torch.Tensor | None = None
            for idx in trange(1, input_data.gop_size):
                ...
                if idx == 1:
                    ...
                    assert prev_res_hat is not None
...
def test(config, dataset, animation_model_arch, kp_detector_arch, **kwargs):
    ...
    if model_id == 'baselines':
        ...
        all_metrics = {}
        qp_for_log = config['eval_params']['qp']
        for x in dataset:
            ...
            codec.create_gops()
            codec.run()
            metrics = monitor.compute_metrics(video[:codec.num_frames], codec.decoded_video)
            metrics['bitrate'] = codec.get_bitrate()
            all_metrics[name] = metrics
        with open(f"{kwargs['log_dir']}/metrics_{qp_for_log}.json", 'w') as f:
