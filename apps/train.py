...
import torch
import os
from training import gen_trainers, disc_trainers
from common.logger import Logger
from torch.utils.data import DataLoader
from data.dataset import DatasetRepeater
...
def _sum_loss_terms(loss_dict):
    """Sum per-key loss tensors (mean each); avoids builtin sum() typing as int."""
    parts = [v.mean() for v in loss_dict.values()]
    ...
    with Logger(log_dir=kwargs['log_dir'], visualizer_params=config['visualizer_params'],
        ...
        for epoch in trange(0, train_params['num_epochs']):
            for x in tqdm(dataloader, leave=False):
                ...
                last_x, last_generated = x, generated
                ...
                loss = _sum_loss_terms(losses_generator)
                loss.backward()
                gen_optimizer.step()
                ...
                if aux_optimizer is not None:
                    aux_loss_tensor = generator.sdc.aux_loss()
                    if generator.tdc is not None:
                        aux_loss_tensor = aux_loss_tensor + generator.tdc.aux_loss()
                ...
                disc_loss = _sum_loss_terms(losses_discriminator)
                disc_loss.backward()
                disc_optimizer.step()
                ...
                if aux_loss_tensor is not None:
                    aux_item = aux_loss_tensor.mean().detach().cpu().item()
                    if aux_item > 0:
                        losses["aux_loss"] = aux_item
            ...
            logger.log_epoch(epoch, state_dict, inp=last_x, out=last_generated)
            if kwargs['debug']:
                break
