"""Video quality assessment metrics for compression evaluation.

Implements PSNR, MS-SSIM, LPIPS, DISTS, FSIM, VMAF, IW-SSIM, and multi-scale
VGG perceptual distance. All metrics operate on frame pairs and support both
per-frame (temporal) and sequence-averaged reporting modes.
"""
import os
import torch
import imageio
...
class MetricParent:
    ...
    def calc(self, orig, rec, *args, **kwargs):
...
class PSNRMetric(MetricParent):
    ...
    def calc(self, orig, rec, weight=None, _lambda=1.0):
        ans = []
        for plane in orig:
...
class MSSSIM_IQA(MetricParent):
    ...
    def calc(self, orig, rec):
        if 'Y' not in orig or 'Y' not in rec:
            return -100.0
        plane = 'Y'
        a = orig[plane].unsqueeze(0).unsqueeze(0)
        b = rec[plane].unsqueeze(0).unsqueeze(0)
...
class VIF_IQA(MetricParent):
    ...
    def calc(self, orig, rec):
        ...
        if 'Y' not in orig or 'Y' not in rec:
            return -100.0
        plane = 'Y'
        b = convert_range(rec[plane].unsqueeze(0).unsqueeze(0), data_range,[0, 1])
        a = convert_range(orig[plane].unsqueeze(0).unsqueeze(0), data_range,[0, 1])
...
class FSIM_IQA(MetricParent):
    ...
    def calc(self, orig: np.ndarray, rec: np.ndarray):  
        ans = 0.0
        # print(orig.shape, rec.shape)
        orig_t = torch.tensor(orig).unsqueeze(0).permute(0,3, 1, 2)
        rec_t = torch.tensor(rec).unsqueeze(0).permute(0,3, 1, 2)
        ans = fsim(orig_t, rec_t).item()
        return ans
...
class NLPD_IQA(MetricParent):
    ...
    def calc(self, orig, rec):
        ...
        if self.chan == 1:
            plane = 'Y'
            target = orig[plane].unsqueeze(0).unsqueeze(0)
            ref = rec[plane].unsqueeze(0).unsqueeze(0)
        else: # self.chan == 3
            target = convert_yuvdict_to_tensor(orig, orig['Y'].device)
            ref = convert_yuvdict_to_tensor(rec, rec['Y'].device)
            
        self.nlpd = self.nlpd.to(target.device)
        ans = self.nlpd(ref, target, as_loss=False).item()
        return ans
...
class VMAF(MetricParent):
    ...
    def calc(self, orig: Dict[str, torch.Tensor], rec: Dict[str, torch.Tensor]) -> float:
        ...
        write_yuv(orig, fp_o.name, self.bits)
        write_yuv(rec, fp_r.name, self.bits)
        ...
        args = [
            ...
            str(orig['Y'].shape[1]), '-h',
            str(orig['Y'].shape[0]), '-p', '420', '-b',
...
class LPIPS_IQA(MetricParent):
    ...
    def calc(self, orig: np.ndarray, rec: np.ndarray, weight=None):  
        ans = 0.0
        compute_weighted = weight is not None
        orig_t = torch.tensor(orig[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
        rec_t = torch.tensor(rec[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
        
        orig_t = to_cuda(orig_t)
        rec_t = to_cuda(rec_t)
        if compute_weighted:
            weight = to_cuda(weight)

        if compute_weighted:
            ans = self.lpips(orig_t, rec_t, weight).item()
        else:
            ans = self.lpips(orig_t, rec_t).item()  
        return ans
...
class DISTS_IQA(MetricParent):
    ...
    def calc(self, orig: np.ndarray, rec: np.ndarray):  
        ...
        if torch.cuda.is_available():
            orig_t = orig_t.cuda()
            rec_t = rec_t.cuda()
        ans = self.dist(orig_t, rec_t).item()
        return ans
...
class MSVGG(MetricParent):
    ...
    def calc(self, orig: np.ndarray, rec: np.ndarray): 	
        ...
        pyramide_real = self.pyramid(orig_t)
        pyramide_generated = self.pyramid(rec_t)
        value_total = 0.0
