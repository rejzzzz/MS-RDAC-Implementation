"""
Neural network building blocks for the codec architecture.

Provides reusable encoder/decoder layers, residual blocks, and the hourglass
module used by the dense motion generator and keypoint detector.
"""
...
class ResBlock2d(nn.Module):
    """Residual block that preserves spatial resolution via skip connections."""

    def __init__(self, in_features: int, kernel_size: Tuple[int, int] = (3, 3), padding: Tuple[int, int] = (1, 1)):
...
class UpBlock2d(nn.Module):
    """Bilinear upsampling followed by convolution, normalization and ReLU."""

    def __init__(self, in_features: int, out_features: int, scale_factor: int = 2,
                 kernel_size: int = 3, padding: int = 1, groups: int = 1):
...
class DownBlock2d(nn.Module):
    """Convolution + BatchNorm + ReLU followed by 2x2 average pooling."""
...
class Encoder(nn.Module):
    """Multi-scale encoder arm of the hourglass network."""

    def __init__(self, block_expansion: int, in_features: int, num_blocks: int = 3,
...
class Decoder(nn.Module):
    """Multi-scale decoder arm with skip connections from the encoder."""
...
class Hourglass(nn.Module):
    """Symmetric encoder-decoder with skip connections for dense prediction."""
